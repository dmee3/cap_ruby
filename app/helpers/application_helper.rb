  ...
  def shell_nav_for(role)
    ...
    when 'admin'
      [
        ...
        NavItem.new(label: 'Users',     path: admin_users_path,           icon: :users,         match: %r{\A/admin/users}),
        NavItem.new(label: 'Payments',  path: admin_payments_path,        icon: :cash,          match: %r{\A/admin/payment}),
        NavItem.new(label: 'Conflicts', path: admin_conflicts_path,       icon: :calendar,      match: %r{\A/admin/conflicts}),
        NavItem.new(label: 'Files',     path: files_path,                 icon: :folder),
        NavItem.new(label: 'Inventory', path: inventory_categories_path,  icon: :cube,          match: %r{\A/inventory/categor}),
    ...
    when 'coordinator'
      [
        ...
        NavItem.new(label: 'Conflicts', path: coordinators_conflicts_path, icon: :calendar, match: %r{\A/coordinators/conflicts}),
        NavItem.new(label: 'Files',     path: files_path,                  icon: :folder),
        NavItem.new(label: 'Inventory', path: inventory_categories_path,   icon: :cube,     match: %r{\A/inventory/categor}),
        NavItem.new(label: 'Emails',    path: inventory_email_rules_path,  icon: :mail)
      ]
    ...
    when 'member'
      [
        ...
        (NavItem.new(label: 'Inventory', path: inventory_categories_path, icon: :cube, match: %r{\A/inventory/categor}) if current_user&.quartermaster?)
      ].compact
    else
      []
    end
