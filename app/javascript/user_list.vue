<template>
  <div class="card">
          ...
          <span class="float-right">
            <div class="dropdown">
              ...
              <div class="dropdown-menu dropdown-menu-right" v-bind:aria-labelledby="`dropdown-${user.id}`">
                <a class="dropdown-item" v-bind:href="`/admin/users/${user.id}`">View Details</a>
                <a class="dropdown-item" v-bind:href="`/admin/users/${user.id}/edit`">Edit Info</a>
                <a class="dropdown-item" v-bind:href="`/admin/payment_schedules/${user.payment_schedule_id}`">Edit Payment Schedule</a>
                <div class="text-danger dropdown-item" @click="showDeleteModal(user)">Delete User</div>
              </div>
            </div>
          </span>
        </a>
    ...
    <div class="modal" id="delete-modal" tabindex="-1" role="dialog" v-if="userType == 'member'">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">Delete {{ userToDelete.first_name + ' ' + userToDelete.last_name }}</h5>
          ...
          <div class="modal-footer" id="delete-form">
            ...
            <input type="submit" value="Delete" class="btn btn-danger" data-dismiss="modal" @click="deleteUser()">
          </div>
        </div>
      </div>
    </div>
    ...
    deleteUser() {
      ...
      $.ajax({
        ...
        .done(function(response) {
          Toast.showToast('Success!', `${self.userToDelete.first_name} was deleted.`, 'success');
          self.getUserList();
        })
        .fail(function(err) {
          Toast.showToast('Whoops!', `Unable to delete ${self.userToDelete.first_name}.`, 'danger');
        });
    },
    getUserList() {
      const self = this;
