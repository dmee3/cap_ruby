<template>
  <div>
    <div class="card-body">
      <h4 class="card-title">Recent Payments<small v-if="this.payments.length > 0" class="text-muted"> (${{ this.totalPaid }})</small></h4>
      <div class="form row">
    ...
    <ul class="list-group list-group-flush">
      <li v-for="payment in payments" v-bind:key="payment.id" class="list-group-item">
        <span class="text-success">${{ payment.amount }}</span> {{ payment.name }}
  ...
  methods: {
    filter: function(event) {
      ...
      $.getJSON('/admin/payments/recent',
        ...
        .fail(err => {
          self.error = err;
          Toast.showToast('Whoops!', `Unable to get details about recent payments.`, 'danger');
          console.log(err);
        });
    }
  }
