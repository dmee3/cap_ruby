  ...
  methods: {
    formatData() {
      ...
      this.payments.forEach((payment) => {
        let day = this.paymentData.find((p) =>
          p.day.isSame(moment(payment.date_paid))
        )
        if (day) {
          day.amount += payment.amount
        } else {
          this.paymentData.push({
            day: moment(payment.date_paid),
      ...
      const chart = new Chart(ctx, {
        ...
        data: {
          datasets: [
            {
              label: 'Actual Amount',
              data: this.paymentData.map((point) => ({
                x: point.day,
                y: point.amount / 100,
              })),
            ...
            },
            {
              ...
              data: this.scheduleData.map((point) => ({
                x: point.day,
                y: point.amount / 100,
              })),
